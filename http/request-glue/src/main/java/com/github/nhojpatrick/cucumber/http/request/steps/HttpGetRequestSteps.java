...
import com.github.nhojpatrick.cucumber.http.request.HttpGetRequest;
...
public class HttpGetRequestSteps
    ...
    @Given("http get to uri {string}")
    public void httpGetToUri(final String uri)
            throws CheckedIllegalArgumentException {

        final UrlWrapper urlWrapper = new UrlWrapperImpl(uri);

        final HttpResponse httpResponse = new HttpGetRequest()
                .get(urlWrapper, this.runState);

        this.runState.set(HTTP_RESPONSE_BODY, httpResponse);
    }

    @Given("http get to url {string}")
    public void httpGetToUrl(final String url)
            throws CheckedIllegalArgumentException {
