@Json
@Map
Feature: Convert To Map Checking Feature with Default keys
  ...
  Scenario: Complex input
    ...
    When I convert object to json map using default RunStateKeys
    Then I have run state key->value string pairs of:
      | runState.json.map | {count=3234, active=true, title=Complex, nested={name=Nested, labels=[labelOne, labelTwo, labelThree], bottom={alpha=Alpha}, nests=[{lnName=nests1, lnId=1, lnBottom={charlie=nests1Charlie}}, {lnName=nests2, lnId=2, lnBottom={charlie=nests2Charlie}}, {lnName=nests3, lnId=3, lnBottom={charlie=nests3Charlie}}]}, tags=[tagOne, tagTwo, tagThree], objs=[{lcName=objs1, lcId=1, lcBottom={bravo=objs1Bravo}}, {lcName=objs2, lcId=2, lcBottom={bravo=objs2Bravo}}, {lcName=objs3, lcId=3, lcBottom={bravo=objs3Bravo}}]} |
